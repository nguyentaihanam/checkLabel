...
namespace Checklabelprinter.Utils
{
    public class GuiLogger : ILogger
    {
        ...
        public GuiLogger(System.Windows.Controls.TextBox logBox, bool enabled, ref bool isGuiClosing)
        {
            _logBox = logBox;
        ...
        private void DisplayEnqueuedMessages()
        {
            ...
            if (message != null)
            {
                if (!_logBox.Dispatcher.CheckAccess())
                    _logBox.Dispatcher.Invoke(new LogHandler(LogToGui_Invoke), new object[] { message });
                else
        ...
        {
            EnqueueMessage(string.Format("{0}: {1} [{2}]\r\n", DateTime.Now.ToLongTimeString(), function, message));
        }

        public void Log_line(string message)
        {
            EnqueueMessage(string.Format("{0}\r\n", message));
        }
